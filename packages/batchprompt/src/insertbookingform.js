...
function createDebugSvg(width, height, grid, blockSize, foundRect) {
    const elements = [];
    // Draw grid blocks
...
async function detectScreenArea(inputPath, options, debugOutputPath) {
    // 1. Analyze image
...
class BookingFormDrawer {
    ...
    logoData;
    formData;
    constructor(width, height, formData, logoData, scalingOptions = { general: 1.0, stepper: 1.0, header: 1.0, content: 1.0, footer: 1.0, logo: 1.0 }) {
        this.width = width;
        this.height = height;
...
async function drawBookingForm(inputPath, outputPath, rect, formData, logoPath, superSample = 8, scalingOptions) {
    // Load logo
    let logoData;
    try {
...
main();
//# sourceMappingURL=insertbookingform.js.map