...
export const createDefaultRegistry = (capabilities) => {
...
export const initConfig = async (overrides = {}) => {
    ...
    const globalContext = {
        ...
        defaultModel,
        contentResolver,
        events: new (await import('eventemitter3')).EventEmitter() // Hack to avoid importing EventEmitter in types.ts if not needed, but we did import it.
    };
    ...
    const actionRunner = new ActionRunner(globalContext, pluginRegistry, preprocessorRegistry, stepResolver, messageBuilder);
    return {
        config,
        globalContext,
        pluginRegistry,
