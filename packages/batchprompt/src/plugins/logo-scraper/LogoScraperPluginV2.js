...
// Plugin
// =============================================================================
export class LogoScraperPluginV2 {
    type = 'logo-scraper';
    configSchema = LogoScraperConfigSchemaV2;
    cliOptions = [
    ...
    async resolveConfig(rawConfig, row, inheritedModel, contentResolver) {
        ...
        const resolveModel = async (prompt, modelOverride, temperatureOverride, thinkingLevelOverride) => {
            let parts = [];
            if (prompt) {
        ...
        return {
            ...
            url,
            analyzeModel: await resolveModel(rawConfig.analyzePrompt, rawConfig.analyzeModel, rawConfig.analyzeTemperature, rawConfig.analyzeThinkingLevel),
            extractModel: await resolveModel(rawConfig.extractPrompt, rawConfig.extractModel, rawConfig.extractTemperature, rawConfig.extractThinkingLevel),
    ...
    async execute(config, context) {
        ...
        const scraper = new AiLogoScraper(puppeteerHelper, analyzeLlm, extractLlm, imageDownloader, {
        ...
        if (result.logos && result.logos.length > 0) {
            ...
            const savedLogoPaths = [];
            const savedFaviconPaths = [];
...
//# sourceMappingURL=LogoScraperPluginV2.js.map