...
const LOGO_FINDER_PROMPT_BASE = (url, siteTitle, maxLogos) => `You are an expert web asset analyst. Your task is to identify sources for the main brand logo from the provided HTML, CSS, and a screenshot of the website.
...
export class AiLogoScraper {
    ...
    constructor(puppeteerHelper, analyzeLlm, extractLlm, imageDownloader, options = {}) {
        this.puppeteerHelper = puppeteerHelper;
        this.analyzeLlm = analyzeLlm;
    ...
    async scrape(url) {
        ...
        try {
            const resolutions = [{ width: 1280, height: 800 }];
            // Step 1: Navigate, dismiss cookies, and take a screenshot.
            const { pageHtml, pageCss, siteTitle, screenshotBase64, finalUrl } = await pageHelper.navigateAndCache(url, async (ph) => {
            ...
            const faviconUrls = results[0].status === 'fulfilled' ? results[0].value : [];
            const faviconUrlSet = new Set(faviconUrls);
            const logoUrls = results[1].status === 'fulfilled' ? results[1].value : [];
            const inlineLogoDataUris = results[2].status === 'fulfilled' ? results[2].value : [];
            ...
            const validLogos = allBase64PngInfo.filter(Boolean);
    ...
    async fetchFavicons(url, pageHelper) {
    ...
    async findLogoUrlsByLlm(url, html, siteTitle, screenshotBase64, cssSnippets) {
        try {
...
Return an array of these source URLs or data URIs.`;
    ...
    async findInlineLogosByLlm(url, html, siteTitle, screenshotBase64, pageHelper) {
        try {
            const maxLogos = this.options.maxLogosToAnalyze ?? 5;
            const mainInstruction = `${LOGO_FINDER_PROMPT_BASE(url, siteTitle, maxLogos)}
...
`;
            ...
            if (Array.isArray(dataUris)) {
                return dataUris.filter((uri) => typeof uri === 'string' && uri.startsWith('data:'));
            }
            return [];
        }
        catch (error) {
    ...
    async normalizeLogos(baseUrl, base64Logos, siteTitle, screenshotBase64) {
...
//# sourceMappingURL=AiLogoScraper.js.map