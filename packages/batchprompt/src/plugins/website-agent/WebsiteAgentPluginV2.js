...
export class WebsiteAgentPluginV2 {
    type = 'website-agent';
    // We use the Loose schema for the plugin interface to allow CLI/File inputs
    configSchema = LooseWebsiteAgentConfigSchemaV2;
    cliOptions = [
    ...
    async normalizeConfig(config, contentResolver) {
    ...
    async resolveConfig(rawConfig, row, inheritedModel, contentResolver) {
        ...
        const resolveModelWithPrompt = async (prompt, defaultPrompt, modelOverride, temperatureOverride, thinkingLevelOverride) => {
            let parts;
            if (prompt) {
        ...
        return {
            ...
            navigatorModel: await resolveModelWithPrompt(rawConfig.navigatorPrompt, DEFAULT_NAVIGATOR, rawConfig.navigatorModel, rawConfig.navigatorTemperature, rawConfig.navigatorThinkingLevel),
            extractModel: await resolveModelWithPrompt(rawConfig.extractPrompt, DEFAULT_EXTRACT, rawConfig.extractModel, rawConfig.extractTemperature, rawConfig.extractThinkingLevel),
            mergeModel: await resolveModelWithPrompt(rawConfig.mergePrompt, DEFAULT_MERGE, rawConfig.mergeModel, rawConfig.mergeTemperature, rawConfig.mergeThinkingLevel)
        };
    }
    async execute(config, context) {
        ...
        const result = await agent.scrapeIterative(config.url, config.extractionSchema, config.schema, {
...
//# sourceMappingURL=WebsiteAgentPluginV2.js.map