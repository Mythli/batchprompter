...
const GmailSenderSubjectVariantSchema = z.object({
    key: z.string().min(1),
    subject: zHandlebars
});

const GmailSenderBodyVariantSchema = z.object({
    key: z.string().min(1),
    body: zHandlebars
});
...
export const GmailSenderConfigSchema = z.object({
    ...
    subject: z.union([
        zHandlebars,
        z.array(GmailSenderSubjectVariantSchema).min(1)
    ]).optional(),
    body: z.union([
        zHandlebars,
        z.array(GmailSenderBodyVariantSchema).min(1)
    ]),
...
export class GmailSenderPlugin extends BasePlugin<GmailSenderConfig, GmailSenderConfig> {
    ...
    normalizeConfig(config: GmailSenderConfig, stepConfig: StepConfig, globalConfig: GlobalConfig): GmailSenderConfig {
        ...
        if (config.evaluateReplies && !evaluationModel) {
            evaluationModel = { model: undefined, temperature: undefined, reasoning_effort: undefined, messages: [] }; // Empty messages, will use default prompt
        }
