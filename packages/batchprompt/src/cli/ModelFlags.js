...
export class ModelFlags {
     ...
     */
    static getOptions(namespace, options = {}) {
     ...
     * @param options Configuration for which flags to include
     */
    static register(program, namespace, options = {}) {
        const prefix = namespace ? `${namespace}-` : '';
        const descPrefix = namespace ? `${namespace} ` : '';
        // Model
     ...
     */
    extract(options, namespace, fallbackNamespace) {
        const toCamel = (s) => {
            return s.replace(/-([a-z0-9])/g, (g) => g[1].toUpperCase());
        };
        const getKey = (ns, suffix) => {
        ...
        const getVal = (suffix) => {
            ...
            if (options[specificKey] !== undefined)
                return options[specificKey];
        ...
        if (think)
            config.thinkingLevel = think;
        const system = getVal('system');
        if (system)
            config.systemSource = system;
        const prompt = getVal('prompt');
        if (prompt)
            config.promptSource = prompt;
        return config;
    }
    /**
     ...
     */
    static extractPluginModel(options, prefix, stepIndex, globalOptions) {
        const getOpt = (suffix) => {
        ...
        if (think) {
            ...
            result.thinkingLevel = globalOptions.thinkingLevel;
        }
...
//# sourceMappingURL=ModelFlags.js.map