 ...
 */
function mergeCliOverrides(fileConfig, options, args, pluginRegistry) {
    ...
    // --- Global Overrides ---
    if (options.model)
        config.globals.model = options.model;
    if (options.temperature !== undefined)
        config.globals.temperature = parseFloat(String(options.temperature));
    if (options.thinkingLevel)
        config.globals.thinkingLevel = options.thinkingLevel;
    if (options.concurrency)
        config.globals.concurrency = parseInt(String(options.concurrency), 10);
    if (options.taskConcurrency)
        config.globals.taskConcurrency = parseInt(String(options.taskConcurrency), 10);
    if (options.tmpDir)
        config.globals.tmpDir = options.tmpDir;
    if (options.output)
        config.globals.outputPath = options.output;
    if (options.dataOutput)
        config.globals.dataOutputPath = options.dataOutput;
    if (options.timeout)
        config.globals.timeout = parseInt(String(options.timeout), 10);
    // --- Limit & Offset Overrides ---
    if (options.inputLimit !== undefined)
        config.globals.inputLimit = parseInt(String(options.inputLimit), 10);
    if (options.inputOffset !== undefined)
        config.globals.inputOffset = parseInt(String(options.inputOffset), 10);
    if (options.limit !== undefined)
        config.globals.limit = parseInt(String(options.limit), 10);
    if (options.offset !== undefined)
        config.globals.offset = parseInt(String(options.offset), 10);
    // --- Step Overrides ---
    ...
    if (args.length > maxStepIndex)
        maxStepIndex = args.length;
    // Check options for step-specific flags (e.g. --model-2)
    Object.keys(options).forEach(key => {
        const match = key.match(/(\d+)(?:[A-Z]|$)/);
        if (match) {
            const stepNum = parseInt(match[1], 10);
            if (stepNum > maxStepIndex)
                maxStepIndex = stepNum;
        }
    });
    ...
    for (let i = 0; i < maxStepIndex; i++) {
        ...
        if (options[`model${stepNum}`])
            step.model.model = options[`model${stepNum}`];
        if (options[`temperature${stepNum}`] !== undefined)
            step.model.temperature = parseFloat(String(options[`temperature${stepNum}`]));
        if (options[`thinkingLevel${stepNum}`])
            step.model.thinkingLevel = options[`thinkingLevel${stepNum}`];
        if (options[`system${stepNum}`])
            step.system = options[`system${stepNum}`];
        if (options[`prompt${stepNum}`])
            step.prompt = options[`prompt${stepNum}`]; // Override prompt flag
        // Output Config
        step.output = step.output || {};
        if (options[`output${stepNum}`])
            step.outputPath = options[`output${stepNum}`]; // This is actually top-level in StepConfig, not inside output object
        if (options[`outputColumn${stepNum}`]) {
            step.output.mode = 'column';
            step.output.column = options[`outputColumn${stepNum}`];
        }
        if (options[`export${stepNum}`])
            step.output.mode = 'merge';
        if (options[`explode${stepNum}`])
        ...
        // Other Step Settings
        if (options[`candidates${stepNum}`] !== undefined)
            step.candidates = parseInt(String(options[`candidates${stepNum}`]), 10);
        if (options[`skipCandidateCommand${stepNum}`])
            step.skipCandidateCommand = true;
        if (options[`aspectRatio${stepNum}`])
            step.aspectRatio = options[`aspectRatio${stepNum}`];
        if (options[`command${stepNum}`])
            step.command = options[`command${stepNum}`];
        if (options[`verifyCommand${stepNum}`])
            step.verifyCommand = options[`verifyCommand${stepNum}`];
        if (options[`timeout${stepNum}`] !== undefined)
            step.timeout = parseInt(String(options[`timeout${stepNum}`]), 10);
        if (options[`jsonSchema${stepNum}`])
            step.schema = options[`jsonSchema${stepNum}`];
        // Judge & Feedback
...
// Main Schema Logic
// =============================================================================
export const createConfigSchema = (pluginRegistry) => z.object({
    fileConfig: z.any(),
    ...
    config.steps.forEach((stepDef, index) => {
        ...
        const modelConfig = {
            ...
            promptSource: stepDef.prompt,
            systemSource: stepDef.system
        };
        ...
        steps.push({
            ...
            judge: stepDef.judge ? {
                ...
                promptSource: stepDef.judge.prompt,
                systemSource: stepDef.judge.system
            } : undefined,
            feedback: stepDef.feedback ? {
                model: stepDef.feedback.model,
                promptSource: stepDef.feedback.prompt,
                systemSource: stepDef.feedback.system
            } : undefined,
