...
export class AiImageSearch {
    ...
    constructor(imageSearch, queryLlm, selector, imagesPerSprite = 4) {
        this.imageSearch = imageSearch;
        this.queryLlm = queryLlm;
        this.selector = selector;
        this.imagesPerSprite = imagesPerSprite;
    }
    async process(row, config) {
        // 1. Generate Queries
        const queries = [];
        if (config.query) {
        ...
        if (this.selector && uniqueSurvivors.length > config.limit) {
            ...
            finalSelection = await this.selectFromPool(this.selector, uniqueSurvivors, config.limit, { phase: 'reduce' });
        }
    ...
    async selectFromPool(selector, images, maxSelected = 1, context = {}) {
        if (images.length === 0)
            return [];
