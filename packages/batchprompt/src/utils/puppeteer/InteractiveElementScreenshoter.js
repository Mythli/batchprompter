...
export class InteractiveElementScreenshoter {
    ...
    async screenshot(target, options = {}) {
        const { maxButtons = 3, maxInputs = 3, maxLinks = 3, createCompositeImage = true, seed = 12345, // Default seed for reproducibility
        ...
        if (shouldClosePageHelper) {
            ...
            await pageHelper.navigateToUrl(target);
    ...
    async processElement(pageHelper, handle, type, elementIndex, states, allScreenshots) {
        try {
            ...
            console.warn(`[Screenshoter] Could not process element ${type} #${elementIndex}: ${e.message}`);
        }
        finally {
            await handle.dispose();
        }
    }
    async screenshotElementState(pageHelper, handle, state, type, elementIndex) {
        try {
            ...
            console.warn(`[Screenshoter] Could not screenshot ${type} #${elementIndex} in ${state} state: ${e.message}`);
            return null;
        }
    }
    async getRandomElements(searchContext, selector, maxCount, type, prng) {
        if (maxCount <= 0)
            return [];
    ...
    async createCompositeImage(screenshots) {
        const PADDING = 20;
        const LABEL_HEIGHT = 40;
        const BG_COLOR = '#f0f0f0';
        // Group screenshots by element type and index
        const groupedScreenshots = new Map();
...
//# sourceMappingURL=InteractiveElementScreenshoter.js.map