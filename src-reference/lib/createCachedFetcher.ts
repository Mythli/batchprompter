...
// It must match the native fetch signature.
export type Fetcher = (
...
export function createCachedFetcher(deps: CreateFetcherDependencies): Fetcher {
    const { cache, prefix, ttl, timeout, userAgent } = deps;
    ...
    return async (url: string | URL | Request, options?: RequestInit): Promise<Response> => {
        ...
        // For all other methods, bypass the cache entirely.
        ...
        if (response.ok) {
            ...
            await cache.set(cacheKey, itemToCache, ttl);
