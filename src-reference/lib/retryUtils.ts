    ...
    for (let attemptNumber = 0; attemptNumber <= maxRetries; attemptNumber++) {
        ...
        try {
            ...
            if (attemptNumber >= maxRetries) {
                throw new Error(`Operation failed on final attempt ${attemptNumber + 1} due to an internal error: ${opError.message}. Last input feedback: ${JSON.stringify(currentFeedbackForOperation)}`);
            }
