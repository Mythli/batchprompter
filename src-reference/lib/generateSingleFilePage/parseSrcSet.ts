...
export interface SrcSetCandidate {
  source: SrcSetSource;
  width?: SrcSetDescriptor;
  density?: SrcSetDescriptor;
  height?: SrcSetDescriptor;
}
...
export default function parseSrcset(input: string): SrcSetCandidate[] {
  ...
  let state: 'in descriptor' | 'in parens' | 'after descriptor';
  let c: string;
  ...
  while (true) {
    ...
    if (!url) {
        // This can happen with trailing commas or whitespace.
        // The main loop condition should handle this, but as a safeguard:
        continue;
    }
  ...
  function parseDescriptors(): void {
    ...
    let i: number;
    const candidate: Partial<SrcSetCandidate> = {};
    let desc = "";
    ...
    for (i = 0; i < descriptors.length; i++) {
      const currentDesc = descriptors[i];
      if (!currentDesc) {
        pError = true;
        continue;
      }
    ...
    if (!pError) {
      if (url) {
          ...
          candidates.push(candidate as SrcSetCandidate);
      }
    } else {
      throw new Error(
