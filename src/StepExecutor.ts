...
export class StepExecutor {
    ...
    async execute(
        ...
        if (config.jsonSchema || config.verifyCommand) {
            ...
            const result = await querier.query(
                ...
                async (message: any, info) => {
                    ...
                    if (images && images.length > 0) {
                        ...
                        if (config.outputPath || config.verifyCommand) {
                            // Save temp for verification or final output
                            const savePath = config.outputPath || path.join(path.dirname(config.outputPath || '.'), `temp_verify_${index}_${stepIndex}.png`);
                    ...
                    if (config.verifyCommand) {
                        const verifyPath = config.outputPath || path.join(path.dirname(config.outputPath || '.'), `temp_verify_${index}_${stepIndex}.${images ? 'png' : 'txt'}`);
            ...
            if (config.outputPath && !config.verifyCommand) {
                await ArtifactSaver.save(result.contentToWrite, config.outputPath);
                console.log(`[Row ${index}] Step ${stepIndex} Saved to ${config.outputPath}`);
            }
            ...
            if (textContent) {
                ...
                if (config.outputPath) {
                    await ArtifactSaver.save(textContent, config.outputPath);
                    console.log(`[Row ${index}] Step ${stepIndex} Text saved to ${config.outputPath}`);
                }
            }
            ...
            if (images && images.length > 0) {
                ...
                if (config.outputPath) {
                    await ArtifactSaver.save(imageUrl, config.outputPath);
                    console.log(`[Row ${index}] Step ${stepIndex} Image saved to ${config.outputPath}`);
                }
            }
        }
        ...
        if (config.postProcessCommand) {
            let filePathForCommand = config.outputPath;
            let isTemp = false;
            ...
            if (!filePathForCommand && contentForColumn) {
                ...
                filePathForCommand = path.join(path.dirname(config.outputPath || '.'), `temp_post_${index}_${stepIndex}${ext}`);
        ...
        if (config.outputColumn && contentForColumn) {
            row[config.outputColumn] = contentForColumn;
        }

        return { role: 'assistant', content: assistantResponseContent };
    }
}
