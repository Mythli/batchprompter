...
class DashboardDrawer {
    ...
    drawTable(startX: number, startY: number) {
        ...
        // Rows - slight offset below header labels
        let rowY = colY + this.s(70);
        table.rows.forEach((row, i) => {
            // Alternating row bg? Maybe just lines
            this.elements.push(
                `<line x1="${startX}" y1="${rowY + this.s(20)}" x2="${
                    startX + width
                }" y2="${rowY + this.s(20)}" stroke="#eee" stroke-width="1" />`
            );
            ...
            rowY += this.s(70); // Increased row spacing for even taller rows
        });
    }
    ...
    render() {
        ...
        this.elements.push(`<defs>
            ...
            <clipPath id="canvasClip">
                <rect x="0" y="0" width="${this.width}" height="${this.height}" rx="${borderRadius}" ry="${borderRadius}" />
            </clipPath>
        </defs>`);
