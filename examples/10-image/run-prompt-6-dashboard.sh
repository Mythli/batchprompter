...
npx tsx src/index.ts generate examples/10-image/data.csv examples/10-image/prompt-6-dashboard \
  ...
  --command "magick '{{file}}' examples/10-image/assets/tablet_in_hands.png -gravity center -composite '{{file}}' && npx tsx src/dashboard_tablet.ts '{{file}}' 'out/10-image/{{industry}}/tablet_data.json' 'out/10-image/{{industry}}/01_MenuBarIcon.svg' 'out/10-image/{{industry}}/user_image.png' 'out/10-image/{{industry}}/color_data.json' '{{file}}.tmp.jpg' && mv '{{file}}.tmp.jpg' '{{file}}' && magick '{{file}}' -resize 1024x1024 -quality 95 '{{file}}'"
