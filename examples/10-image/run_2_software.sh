...
do
  ...
  npx tsx src/index.ts generate examples/10-image/data.csv examples/10-image/prompt-2-butlerapp-software \
    --output-1 "out/10-image/{{industry}}/04_AboutCourseFirstImage-$i.png" \
    --aspect-ratio-1 "1:1" \
    --model "google/gemini-3-pro-image-preview" \
    --command "npx tsx src/insertbookingform.ts '{{file}}' 'out/10-image/{{industry}}/form_data.json' 'examples/10-image/logo.png' '{{file}}.tmp' && mv '{{file}}.tmp' '{{file}}'"
done
