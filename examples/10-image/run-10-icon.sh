...
npx tsx src/index.ts generate examples/10-image/data.csv examples/10-image/prompt-10-icon \
  ...
  --command "magick '{{file}}' -trim +repage -background none -gravity center -extent '%[fx:max(w,h)]x%[fx:max(w,h)]' -fuzz 90% -transparent white '{{file}}' && pngquant 2 --nofs --force --output '{{file}}' '{{file}}' && ./examples/10-image/vectorize.sh '{{file}}' && rm '{{file}}'"
